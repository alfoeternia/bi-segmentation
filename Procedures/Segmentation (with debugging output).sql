...
AS
	...
	LRS_ROUTE HPMS_SEGMENTED.geometry%TYPE;

	segment HPMS_SEGMENTED%ROWTYPE;
	remaining_section HPMS_SEGMENTED.geometry%TYPE;
			...
			ELSIF segment.BEG_POINT <= section_begin_point THEN
				...
				IF segment.BEG_POINT = section_begin_point THEN
					...
					ELSIF section_end_point < segment.END_POINT THEN
						...
						UPDATE HPMS_SEGMENTED SET GEOMETRY = SDO_LRS.CONVERT_TO_STD_GEOM(second_segment) WHERE OBJECTID = currentID;
